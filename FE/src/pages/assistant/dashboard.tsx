...
export default function AssistantDashboard() {
    ...
    const [assistantName, setAssistantName] = useState("Phụ xe");
    ...
    // Attendance upload image states
    const [uploadModalOpen, setUploadModalOpen] = useState(false);
    const [selectedTripIdForCheckIn, setSelectedTripIdForCheckIn] = useState<string | null>(null);
    ...
    useEffect(() => {
        ...
        if (userStr && userStr !== "undefined") {
            try {
                ...
                if (user.displayName) {
                    setAssistantName(user.displayName);
                }
                if (user.staffId) {
    ...
    const handleConfirmTrip = async (tripId: string) => {
        try {
            const res = await axios.post(`http://localhost:3000/api/trip/confirm/${tripId}`, {
                staffId,
            });
            message.success(res.data.message || "Xác nhận chạy chuyến thành công!");
            // Reload trips to update driverConfirmed status
            if (staffId) {
                loadTrips(staffId);
            }
        } catch (err: any) {
            message.error(err.response?.data?.message || "Xác nhận chạy chuyến thất bại");
        }
    };

    // Attendance handlers
    const handleAttendanceCheckIn = async (tripId: string, file: any) => {
        ...
        try {
            ...
            setAttendanceMap(prev => ({
                ...
                [tripId]: {
                    ...
                    proofImage: res.data.data?.proofImage,
                },
            }));
            setUploadModalOpen(false);
            setFileList([]);
            setSelectedTripIdForCheckIn(null);
        } catch (err: any) {
    ...
    const startScanner = async (cameraId: string) => {
        ...
        try {
            ...
            setTimeout(async () => {
                try {
                    const instance = new Html5Qrcode("assistant-qr-reader");
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_: any, record: any) => {
                ...
                // Chuyến đã hoàn thành hoặc huỷ thì không hiện nút xác nhận
                const isTripDone = record.status === "hoàn thành" || record.status === "huỷ";
                ...
                return (
                    <Space>
                        ...
                        <Button type="link" size="small" onClick={() => navigate(`/phuxe/trip/${record._id}`)}>
                            Chi tiết
                        </Button>
                    </Space>
                );
        ...
        <AssistantLayout>
            <div style={{ padding: 24 }}>
                ...
                <Row justify="space-between" align="middle" style={{ marginBottom: 32 }}>
                    <Col>
                        <Title level={2}>{greeting}, {assistantName}!</Title>
                        <Text type="secondary">
                ...
                <Row gutter={[24, 24]}>
                    {/* Active Trips */}
                    <Col xs={24}>
                        <Card title={<Text strong style={{ color: "#52c41a" }}>DANH SÁCH CHUYẾN XE HÔM NAY & SẮP TỚI CỦA {assistantName.toUpperCase()}</Text>}>
                ...
                {/* Weekly Performance */}
                <Card
                    title="Hiệu suất phụ xe hàng tuần"
                    style={{ marginTop: 24 }}
                >
                    <Row gutter={[24, 24]}>
                        <Col xs={24} md={12}>
                ...
                <Modal
                    title="Tải lên ảnh minh chứng để chấm công"
                    ...
                    onOk={async () => {
                        if (fileList.length === 0) {
                            message.warning("Vui lòng chọn ảnh minh chứng!");
                            return;
                        }
                        if (selectedTripIdForCheckIn) {
                            await handleAttendanceCheckIn(selectedTripIdForCheckIn, fileList[0]);
                        }
                    }}
                    okText="Xác nhận Chấm công"
                    cancelText="Hủy"
                ...
                >
                    <div style={{ padding: "16px 0", textAlign: "center" }}>
                        <Upload
                            ...
                            listType="picture"
                        >
                            <Button icon={<UploadOutlined />}>Chọn ảnh minh chứng</Button>
                        </Upload>
                        <div style={{ marginTop: 12 }}>
                            <Text type="secondary" style={{ fontSize: 12 }}>
                                * Bắt buộc tải lên ảnh xe hoặc vị trí để minh chứng.
                            </Text>
                        </div>
                    </div>
                </Modal>
                ...
                >
                    <div className="space-y-4 pt-2 flex flex-col items-center">
                        ...
                        <div className="scanner-container">
                            <div id="assistant-qr-reader" className="w-full h-full"></div>
                            {isScanning && (
                                <>
                            ...
                            columns={[
                                ...
                                },
                                {
                                    ...
                                    render: (_: any, record: any) => {
                                        ...
                                        if (selectedTripForList && selectedTripForList.departureTime) {
                                            const now = dayjs();
                                            const departure = dayjs(selectedTripForList.departureTime);
                                            const diffMinutes = now.diff(departure, "minute");
