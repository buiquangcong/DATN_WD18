...
export function Navbar({ isDarkMode, toggleDarkMode }: NavbarProps) {
        ...
        if (userObj?.staffId) {
          fetch(`http://localhost:3000/api/staff/detail/${userObj.staffId}`)
            ...
            .then((data) => {
              ...
              if (staffData) {
                setAssistantUser((prev: any) => ({
                  ...prev,
                  ...staffData,
                  displayName: staffData.ten || prev?.displayName || prev?.username || "Phụ xe",
  ...
  const handleLogout = () => {
    ...
    window.location.href = "/phuxe/login";
  };

  const displayName = assistantUser?.displayName || assistantUser?.ten || assistantUser?.username || "Phụ xe";
  ...
  const items: MenuProps["items"] = [
    {
      ...
      icon: <UserOutlined />,
      onClick: () => navigate("/phuxe/profile"),
    },
    {
      type: "divider",
    },
  ...
  const menus = [
    { title: "Trang chủ", path: "/phuxe" },
    { title: "Danh sách chuyến xe", path: "/phuxe/list" },
    { title: "Phản hồi chuyến xe", path: "/phuxe/feedback" },
  ];
  ...
  return (
    <nav className="fixed top-0 w-full z-50 h-20 flex items-center bg-white/75 dark:bg-inverse-surface/75 backdrop-blur-md border-b border-outline-variant/30 shadow-sm">
      <div className="flex justify-between items-center px-margin-desktop max-w-container-max mx-auto w-full">
        ...
        <NavLink to="/phuxe" className="cursor-pointer">
          ...
          {isLoggedIn ? (
            <Dropdown menu={{ items }} placement="bottomRight" arrow>
              <div className="flex items-center gap-2 cursor-pointer p-1.5 rounded-full hover:bg-black/5 dark:hover:bg-white/10 transition-colors">
                <Avatar
                  src={assistantUser?.avatar || assistantUser?.image}
                  icon={!assistantUser?.avatar && !assistantUser?.image ? <UserOutlined /> : undefined}
                  className="bg-primary text-white font-bold"
                >
                  {!assistantUser?.avatar && !assistantUser?.image && displayName.charAt(0).toUpperCase()}
                </Avatar>
